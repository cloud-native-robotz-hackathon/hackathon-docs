...
. Login into <GOGS_URL>>[Gogs^] with `*<GUID>*` and the provided password.
. Click on the `*+*` in the upper right and choose `*New Migration*`.
. In the `*Clone Address*` field write `*https://github.com/cloud-native-robotz-hackathon/starter-app-php*`
...
In <CHE_URL>/[Eclipse Che^] open your `Workspace` click on `*Import Project*`, choose `GIT` as Version Control System.

In the `*URL*` field paste your GIT Project from Gogs. It should be: `*<GOGS_URL>/<GUID>/<GUID>-php*`.

Edit the link and add your username and passwort to it. It should look like this: `*http://<GUID>:<PASSWORD>@<GOGS_URL>>/<GUID>/<GUID>-php*`
...
Open the <OCP_URL>/console[OpenShift Web UI^]. If not already logged in, log in with `*<GUID>*` and the provided password.

. Click on the `*<GUID>-project*`
. Click on `*Browse Catalog*`, `*Languages*`, `*PHP*`, `*PHP*` again.
. Click on `*Next*`
. As `Application Name` use `*<GUID>-php*` and as `GIT Repository` use `*<GOGS_URL>>/<GUID>/<GUID>-php*`
